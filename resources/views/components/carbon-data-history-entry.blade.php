...
<div x-data="{ entryShowing: false, conformationModalDisplaying: false }" class="flex flex-col m-1 shadow-xl rounded w-full bg-slate-100 outline-1 outline-black"
    :class="{
        'bg-slate-200': entryShowing,
    ...
    <div x-show="entryShowing" class="flex flex-col mt-2 bg-slate-200">
        ...
        {{-- <button class="bg-emerald-600 rounded p1 w-fit self-center m-2"
        wire:click="deleteEntry({{ $tableRow->id }})">Delete</button> --}}
        <button class="bg-emerald-500" x-on:click='conformationModalDisplaying = true'>TEST</button>
    </div>
</div>
