<div x-data="{ comapreModal: false, comapreToModal: false }">
    <section class="flex flex-col md:flex-row gap-5 items-start md:items-center p-1">
        <div class="w-full md:w-auto">
            <h2 class="text-lg font-semibold mb-2">Select Data Type</h2>
            <select 
        ...
        <div class="flex flex-col w-full md:w-auto">
            <div class="relative">
                <h2 class="text-lg font-semibold mb-2">Compare</h2>
                <button x-on:click="comapreModal = ! comapreModal"
                    class="text-black outline outline-offset-1 outline-1 bg-white py-2 px-4 rounded w-full">
                    {{ $compare_entry ? $compare_entry->created_at : 'Select an option' }}
                </button>
                <div x-show="comapreModal" class="absolute z-10 mt-1 w-full rounded-md overflow-y-auto bg-white shadow-lg">
                    @foreach ($data_history as $option)
                        <button wire:click="compare({{ $option->id }})" x-on:click="comapreModal = ! comapreModal"
        ...
        <div class="flex flex-col w-full md:w-auto">
            <div class="relative">
                <h2 class="text-lg font-semibold mb-2">Compare Against</h2>
                <button x-on:click="comapreToModal = ! comapreToModal"
                    class="text-black outline outline-offset-1 outline-1 bg-white py-2 px-4 rounded w-full">
                    {{ $compare_to_entry ? $compare_to_entry->created_at : 'Select an option' }}
                </button>
                <div x-show="comapreToModal" 
                class="absolute z-10 mt-1 w-full rounded-md overflow-y-auto bg-white shadow-lg">
                    @foreach ($compare_to_entries as $option)
                        <button wire:click="compareAgainst({{ $option->id }})" x-on:click="comapreToModal = ! comapreToModal"
                            class="w-full text-left px-4 py-2 hover:bg-gray-100">
                            {{ $option->created_at }}
                        </button>
                    @endforeach
        ...
        <div class="flex w-full md:w-auto">
            <button class="bg-emerald-500 text-white p-2 rounded-xl w-full md:w-auto" wire:click='clearComparison'>Clear Comparison</button>
        </div>
    </section>
    <section>
    </section>
        ...
        <table class="min-w-full bg-white border border-gray-300 rounded-lg shadow-lg">
            <thead>
                <tr class="bg-emerald-500 text-white">
                    <th class="py-3 px-2 sm:px-5 border-b">Electricity</th>
                    <th class="py-3 px-2 sm:px-5 border-b">Natural Gas</th>
                    <th class="py-3 px-2 sm:px-5 border-b">Heating Oil</th>
                    <th class="py-3 px-2 sm:px-5 border-b">Coal</th>
                    <th class="py-3 px-2 sm:px-5 border-b">LPG</th>
                    <th class="py-3 px-2 sm:px-5 border-b">Propane</th>
                    <th class="py-3 px-2 sm:px-5 border-b">Wood</th>
                    <th class="py-3 px-2 sm:px-5 border-b">Co2e</th>
                    <th class="py-3 px-2 sm:px-5 border-b">Created at</th>
                </tr>
            </thead>
            <tbody>
                @if (count($comparison_entries) < 2)
                    @foreach ($data_history as $history)
                        <tr class="hover:bg-gray-100">
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $history->electricity }} {{ $history->electricity_metric }}</td>
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $history->natural_gas }} {{ $history->natural_gas_metric }}</td>
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $history->heating_oil }} {{ $history->heating_oil_metric }}</td>
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $history->coal }} {{ $history->coal_metric }}</td>
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $history->lpg }} {{ $history->lpg_metric }}</td>
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $history->propane }} {{ $history->propane_metric }}</td>
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $history->wood }} {{ $history->wood_metric }}</td>
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $history->total_co2e }}</td>
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $history->created_at }}</td>
                        </tr>
                    @endforeach
                @else
                    @foreach ($comparison_entries as $comparison_entry)
                        <tr class="hover:bg-gray-100">
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $comparison_entry->electricity }} {{ $comparison_entry->electricity_metric }}
                                <strong class="{{ substr($comparison_entry->electricity_diff, 0, 1) == '-' ? 'text-emerald-400' : 'text-red-400' }}">{{ $comparison_entry->electricity_diff }}</strong>
                            </td>
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $comparison_entry->natural_gas }} {{ $comparison_entry->natural_gas_metric }}
                            ...
                            <td class="py-3 px-2 sm:px-5 border-b text-center">{{ $comparison_entry->heating_oil }} {{ $comparison_entry->heating_oil_metric }}
